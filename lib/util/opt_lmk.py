...
class OptLandmark(nn.Module):
    ...
    def load_lmk_3D(self):
        ...
        lmk_3D = torch.Tensor(lmk_3D.T).float().unsqueeze(0)
    ...
    def load_lmk_gt(self, lmk_gt_path):
        ...
        return torch.from_numpy(lmk_gt).float().unsqueeze(0)

    def save_param(self, outputpath):
        dic = {
            'ortho_ratio': self.ortho_ratio, 
            'scale': (self.scale + self.scale_dis).detach().numpy(), 
            'center': (self.center + self.center_dis).detach().numpy(), 
            'R': (self.rotation + self.rotation_dis).detach().numpy()
        }
    ...
    def get_camera(self):
        ...
        extrinsic = torch.cat((extrinsic, torch.Tensor([0, 0, 0, 1]).reshape(1, 4)), 0)
        scale_intrinsic = torch.eye(4)
        scale_intrinsic[0, 0] = scale / ortho_ratio
        scale_intrinsic[1, 1] = -scale / ortho_ratio
        scale_intrinsic[2, 2] = scale / ortho_ratio
        uv_intrinsic = torch.eye(4)
        uv_intrinsic[0, 0] = 1.0 / float(self.rendering_load_size // 2)
        uv_intrinsic[1, 1] = 1.0 / float(self.rendering_load_size // 2)
        uv_intrinsic[2, 2] = 1.0 / float(self.rendering_load_size // 2)
        trans_intrinsic = torch.eye(4)
    ...
    def get_img_lmk(self):
        gt_lmk_coor = np.floor(self.lmk_gt.detach().numpy()[0]).astype(np.int32)
        pred_lmk_coor = np.floor(self.lmk_2D.detach().numpy()[0]).astype(np.int32)
