...
def hair_synthesis(net, device, root_tensor, calib_tensor, num_sample=100, hair_unit=0.006):
    #root:[3, 1024]
    num_strand = root_tensor.shape[2]
    hair_strands = torch.zeros(num_sample, 3, num_strand).to(device=device)
...
def hair_synthesis_DSH(net, device, root_tensor, calib_tensor, num_sample=100, hair_unit=0.006, threshold=[60, 150]):
    ...
    for i in range(1, num_sample - 1):
        ...
        if i > 1:
            ...
            orien_zeros = torch.zeros_like(curr_node_orien).float().to(device=device)
...
def export_hair_real(net, device, data, mesh_path, save_path):
    image_tensor = data['hairstep'].to(device=device).unsqueeze(0)
    calib_tensor = data['calib'].to(device=device).unsqueeze(0)
    root_tensor = torch.from_numpy(get_hair_root()).to(device=device).float().unsqueeze(0)

    net.filter(image_tensor)

    strands = hair_synthesis(net, device, root_tensor, calib_tensor, num_sample=100, hair_unit=0.006)
