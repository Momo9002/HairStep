...
def opt_cam(sample_info):
    ...
    lmk_opt = OptLandmark(target_lmk, input_image)  # Removed .cuda()
...
if __name__ == '__main__':
    ...
    sample_info = [(os.path.join(img_dir, item), os.path.join(lmk_dir, item[:-3]+'npy'),
                    os.path.join(param_dir, item[:-3]+'npy'), os.path.join(lmk_proj_dir, item)) for item in items]
