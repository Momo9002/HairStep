...
def img2strand(opt):
    ...
    # Initialize the model and load state dictionary on the CPU
    model = Model()
    ...
    for item in tqdm(items):
        ...
        # Process the image tensor on the CPU
        rgb_img = Variable(torch.from_numpy(rgb_img).permute(2, 0, 1).float().unsqueeze(0))
